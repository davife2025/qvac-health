...
interface SOAPSemanticSearchProps {
  /** If set, scopes search to one patient */
  patientRef?: string;
  ...
  return (
    <div className="flex items-center gap-1.5">
      <div className="h-1 w-16 rounded-full bg-gray-100 overflow-hidden">
        <div
          className="h-full rounded-full bg-calm-400"
          style={{ width: `${pct}%` }}
        />
      </div>
...
export function SOAPSemanticSearch({
  patientRef,
  onResultClick,
}: SOAPSemanticSearchProps) {
    ...
    <div className="space-y-4">
      ...
      <div className="flex flex-wrap gap-1.5">
        {EXAMPLE_QUERIES.map((q) => (
          <button
            ...
            className="rounded-full bg-gray-100 px-2.5 py-1 text-xs text-gray-500 hover:bg-calm-100 hover:text-calm-700 transition-colors"
          >
            {q}
          </button>
        ))}
      ...
      {results.length > 0 && (
        <div className="space-y-2">
          ...
          {results.map((r, i) => {
            // Extract note content below the header lines
            ...
            return (
              <div
                ...
                className={`
                  rounded-xl bg-white px-4 py-3 ring-1 ring-gray-100 space-y-2
                  ${onResultClick ? "cursor-pointer hover:ring-calm-200 hover:bg-calm-50 transition-colors" : ""}
                `}
              >
                <div className="flex items-center justify-between gap-2">
                  <p className="text-[10px] text-gray-400 truncate">{header}</p>
                  {scoreBar(r.score)}
                </div>
      ...
      {!searching && query.trim() && results.length === 0 && (
        <p className="text-sm text-gray-400 text-center py-4">
          No matching notes found. Try a different concept.
        </p>
      )}
    </div>
  );
