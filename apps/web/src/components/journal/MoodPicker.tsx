      ...
      <div className="flex gap-1.5 sm:gap-2">
        {MOODS.map((mood) => (
          <button
            ...
            aria-pressed={value === mood.level}
            className={`
              flex flex-1 flex-col items-center justify-center gap-0.5 sm:gap-1
              rounded-xl border-2 py-3 sm:py-2.5 px-1
              min-h-[52px] sm:min-h-[auto]
              text-center transition-all duration-150
              ${value === mood.level
                ? `${mood.color} border-2 scale-105 shadow-sm`
                : "border-gray-200 bg-white hover:border-gray-300 active:scale-95"}
              disabled:opacity-50 disabled:cursor-not-allowed disabled:active:scale-100
            `}
          >
            <span className="text-xl sm:text-xl leading-none">{mood.emoji}</span>
            {/* Label hidden on very small screens, shown sm+ */}
            <span className="hidden sm:block text-[10px] font-medium leading-none text-gray-500">
              {mood.label}
            </span>
          </button>
        ))}
  ...
  return (
    <span
      className={`inline-flex items-center gap-1 rounded-full px-2 py-0.5
        text-xs font-medium border ${m.color}`}
    >
      {m.emoji} {m.label}
    </span>
  );
